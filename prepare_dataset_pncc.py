...
def preprocess_dataset(dataset_path, json_path, num_ceps = 13, low_freq = 0, high_freq = 2000, nfilts = 24, nfft = 512, dct_type = 2, use_energy = False,lifter = 5,normalize = False):
    """Extracts PNCCs from music dataset and saves them into a json file.
        ...
        if dirpath is not dataset_path:
            ...
            for f in filenames:
                ...
                if len(sig) >= SAMPLES_TO_CONSIDER:
                    ...
                    # compute features
                    NGCCs = ngcc(sig=sig, fs=fs,num_ceps=num_ceps, nfilts=nfilts, nfft=nfft, low_freq=low_freq, high_freq=high_freq, dct_type=dct_type, use_energy=use_energy, lifter=lifter, normalize=normalize)

                    # store data for analysed track
                    data["NGCCs"].append(NGCCs.tolist())
